...
public class BossServiceMethodRepo : IBossServiceMethodRepo
{
    ...
    public BossServiceMethodRepo(NpgsqlDataSource ds)
    {
        _ds = ds;
    }

    public async Task<List<BossServiceMethodVm>> GetBossMethodsAsync(Guid userId, CancellationToken ct)
    {
        const string sql = @"
        ...
        var rows = await conn.QueryAsync<BossServiceMethodVm>(sql, new { userId });
        return rows.ToList();
    }

    public async Task UpdateBossMethodsAsync(Guid userId, List<string> methods, CancellationToken ct)
    {
        using var conn = await _ds.OpenConnectionAsync(ct);
